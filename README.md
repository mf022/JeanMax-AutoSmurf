...
**Special thanks**  [@JeanMax](https://github.com/JeanMax/) for releasing this script, and to [@dark-f](https://d2bot.discourse.group/u/hk-dark-f/) who updated in 2018 and added many extra files.
...
1. when your bot team get the baalLvlnm, the AutoSmurf will enter on Hell difficulty and you should use x2 char-config files, so that you should copy those files using your char names for x2, the same like in x1 case.

2. in x2 files, Config.AutoEquip = false is by default, so  further more your bot should be equiped manually by yourself. 

### Leader

1. Leader bot should be a sorceress class, and if your char name is **AbC**, you should copy /config/Sorceress.x1(leader).js as **Sorceress.AbC.js**.

2. complete the lines 16-20 with the required character and profile names:
	```javascript
		Scripts.AutoSmurf = true;
			Config.AutoSmurf.TeleSmurf = "AbC"; // your leader sorceress charname.
			Config.AutoSmurf.BoSmurf = "BcD"; // BoSmurf charname
			Config.AutoSmurf.OtherSmurf = ["CdE", "DeF"]; // all other team charnames, excluding the leader and BoSmurf
...
1. you should copy the follower configuration files:
	- the 2nd required **BcD** follower is BO barbarian, so you should copy /config/Barbarian.x1.js as **Barbarian.BcD.js**.
	- the 3rd follower **CdE** can be a paladin, so you should copy /config/Paladin.x1.js as **Paladin.CdE.js**.
	- the 4th follower **DeF** could be a sorceress, so you should copy /config/Sorceress.x1(follow).js as **Sorceress.DeF.js**.

2. complete the lines 16-20, like in the leader's case.
	```javascript
		Scripts.AutoSmurf = true;
			Config.AutoSmurf.TeleSmurf = "AbC"; // your leader sorceress charname.
			Config.AutoSmurf.BoSmurf = "BcD"; // BoSmurf charname
...
4. You may reset the values of [lines 15-20 in AutoSmurf.js](https://github.com/JeanMax/AutoSmurf/blob/master/d2bs/kolbot/libs/bots/AutoSmurf.js#L15-L20). These values restrict your bot level to begin some quests, like "tombsLvl: [22, -1, -1]," (line 17) means that your bot level must be more than 22 to kill Duriel in normal difficulty. Because the beginner bots equips mostly low quality items, if you want to reduce the number of chicken exits, the suggestion is to not change the default values. The default values will keep your bot safety, but on lower areas the leveling up speed is a bit slower.
