...
*	@author		JeanMax
*	@contributors	Dark-f, Alogwe, Imba, Kolton, Larryw, Noah, QQValpen, Sam and YGM  
...
var Team = {
	size: 2 + Config.AutoSmurf.OtherSmurf.length,
	type: {all: (1 | 2 | 4), TeleSmurf: 1, BoSmurf: 2, OtherSmurf: 4},
	myType: 4,
	names: [],
	...
	init: function () {
		...
		this.start = function () {
			...
			if (me.windowtitle === Config.AutoSmurf.TeleSmurf) {
				this.myType = this.type.TeleSmurf;
			} else if (me.windowtitle === Config.AutoSmurf.BigSmurf) {
				this.myType = this.type.BoSmurf;
			}
			...
			if (this.myType === this.type.BoSmurf) {
			...
			this.goToBo = function () {
				...
				if (!Pather.accessToAct(2)) {
					...
					if (getWaypoint(10)) {
						...
						Town.goToTown(2);
						if (this.myType === this.type.TeleSmurf) {
				...
				if (this.myType === this.type.TeleSmurf) {
					...
					if (!this.inArea()) {
						...
						this.sendMsg(this.type.BoSmurf | this.type.OtherSmurf,
										"come");
					}
				}
	...
	sendMsg: function (teamType, action, x, y, area) {
		...
		this.sendTo = function (mode, msg) {
			if ((teamType & this.type.TeleSmurf) !== 0) {
				sendCopyData(null, Config.AutoSmurf.TeleSmurf, mode, msg);
			}
			if ((teamType & this.type.BoSmurf) !== 0) {
				sendCopyData(null, Config.AutoSmurf.BigSmurf, mode, msg);
			}
			if ((teamType & this.type.OtherSmurf) !== 0) {
				for (i = this.size - 3; i >= 0; i -= 1) {
					sendCopyData(null, Config.AutoSmurf.OtherSmurf[i], mode, msg);
				}
			}
		...
		if (x && y && area) { //updating coords (Util.startFollow)
			...
			switch (this.myType) {
			case this.type.BoSmurf:
				this.sendTo(105, "*" + me.name);
				break;
			case this.type.TeleSmurf:
				this.sendTo(105, "$" + me.name);
				break;
			default:
				this.sendTo(105, me.name);
	...
	sync: function (timeout, seed) {
		...
		if (this.myType === this.type.BoSmurf) {
			...
			while (timeout && Msg.x !== this.size - 1) {
				...
				Msg.x = 0;
				this.sendMsg(this.type.TeleSmurf | this.type.OtherSmurf, "ping");
				delay(50);
				timeout -= 1;
			...
			Msg.x = save;
			if (timeout) {
				this.sendMsg(this.type.TeleSmurf | this.type.OtherSmurf, seed);
			}
		...
		for (i = 0; i < wpIDs.length; i += 1) {
			...
			if (preset) {
				...
				try {
					...
					Town.move("portalspot");
					delay(10000);
					this.sendMsg(this.type.TeleSmurf | this.type.BoSmurf,
									"tp");
					for (k = 0; k < 1000; k += 1) {
				...
				if (wp) {
					if (!tob) {
						if (this.myType === this.type.BoSmurf && wait) {
							for (j = 0; j < 60 && !this.inArea(); j += 1) {
								Attack.clear(20);
								delay(500);
							}
							this.sendMsg(this.type.OtherSmurf, "wp");
						}
						tob = true;
					}
					for (j = 0; j < 10; j += 1) {
	...
	act3to4: function () {
		...
		if (this.myType === this.type.TeleSmurf) {
			...
			try {
				...
				Pather.makePortal();
				this.sendMsg(this.type.OtherSmurf | this.type.BoSmurf, "come");
			} catch (e) {
				print(e);
				return false;
			}
		} else {
			this.sendMsg(this.type.TeleSmurf, "mephTP");
			Town.move("cain");
...
var Util = {
	...
	OtherSmurfToExit: function (targetArea, use, clearPath) {
	...
	startFollow: function () {
		...
		this.checkExit = function (area) { //thanks kolton! :D
			...
			for (i = 0; i < exits.length; i += 1) {
				if (exits[i].target === area) {
					this.OtherSmurfToExit(area, true, Config.ClearType);
					return true;
				}
			}
			...
			return this.OtherSmurfToExit(area, true, Config.ClearType);
		};
		...
		Msg.area = 0;
		Msg.action = "go";
		if (Team.myType !== Team.type.OtherSmurf) {
			return false;
		}
		...
		while (Msg.action !== "stop") {
			...
			if (i === 3) {
				Team.sendMsg(Team.type.TeleSmurf | Team.type.BoSmurf,
								"update");
				delay(200);
			} else {
			...
			if (Msg.area && me.area && Msg.area !== me.area) {
				if (!this.checkExit(Msg.area)) {
					...
					for (i = 0; i < 120 && me.inTown; i += 1) {
						Team.sendMsg(Team.type.BoSmurf, "tp");
						delay(500 + me.ping * 2);
	...
	stopFollow: function () {
		...
		Msg.action = "stop";
		if (Team.myType !== Team.type.OtherSmurf) {
			Team.sendMsg(Team.type.OtherSmurf, "stop");
		}
	},
		...
		if (j < areaIDs.length) {
			...
			while (j < areaIDs.length) {
				...
				case 53:// Palace -> palace...
					...
					this.OtherSmurfToExit(areaIDs[j], true, Config.ClearType);
					break;
...
var Quest = {
	act1: function () {
		this.den = function () {
			...
			if (!me.getQuest(1, 1)) {
				Util.OtherSmurfToExit([1, 2], true, Config.ClearType);
				Util.startFollow();
				if (Team.myType !== Team.type.OtherSmurf) {
					Util.OtherSmurfToExit([2, 3], true, Config.ClearType);
					delay(500);
					Team.clickWP();
					Util.OtherSmurfToExit([2, 8], true, Config.ClearType);
					Team.inGame();
					for (cleartry = 1;
				...
				Util.stopFollow();
				Util.OtherSmurfToExit([2, 1], true, Config.ClearType);
				if (!me.inTown) {
					delay(1000);
					Packet.flash(me.gid);
					delay(500);
					Util.OtherSmurfToExit([2, 1], false);
					Util.OtherSmurfToExit([2, 1], true, Config.ClearType);
				}
			}
		...
		this.cave = function () {
			...
			Util.startFollow();
			if (Team.myType !== Team.type.OtherSmurf) {
				Util.OtherSmurfToExit([9, 13], true, Config.ClearType);
			...
			if (Team.isLevel(SmurfConfig.caveLvl[me.diff])) {
				if (Util.OtherSmurfToExit([9, 3], true, Config.ClearType)) {
					Team.clickWP(false);
		...
		this.andy = function () {
			...
			if (!me.getQuest(6, 1)) {
				...
				if ((Team.myType === Team.type.TeleSmurf && me.diff)
						|| (Team.myType !== Team.type.OtherSmurf && !me.diff)) {
					try {
						Util.travel(1);
					} catch (e1) {
						Attack.clearLevel();
					...
					if (!Pather.teleport
							|| !Util.OtherSmurfToExit([36, 37], true,
												Config.ClearType)) {
						Util.OtherSmurfToExit([36, 37], true);
					}
				...
				if (me.diff !== 2) {
					Pather.teleport = false;
					Pather.makePortal();
					Team.sendMsg(Team.type.OtherSmurf | Team.type.BoSmurf, "come");
	...
	act2: function () {
		this.cube = function () {
			...
			this.lead = function () {
				...
				for (i = 0; i < 3; i += 1) {
					if (me.area === 42) {
						Util.OtherSmurfToExit(56, true, Config.ClearType);
					}
					if (me.area === 56) {
						Util.OtherSmurfToExit(57, true, Config.ClearType);
					}
					if (me.area === 57) {
						Team.clickWP();
						Util.OtherSmurfToExit(60, true, Config.ClearType);
					}
					if (me.area === 60) {
						this.toChest();
					}
			...
			if (Team.myType === Team.type.TeleSmurf || !me.diff) {
				...
				if (!me.diff) {
					...
					while (me.area !== 61) {
						...
						Util.OtherSmurfToExit(path[path.indexOf(me.area) + 1],
					...
					Pather.teleport = true;
					Config.ClearType = false;
					Util.OtherSmurfToExit([45, 58, 61], true);
		...
		this.staff = function () {
			...
			Util.OtherSmurfToExit([62, 63, 64], true);
		...
		this.summoner = function () {
			...
			if (Team.myType === Team.type.TeleSmurf) {
				if (me.area !== 74) {
					Town.goToTown();
					Town.move("waypoint");
				...
				Team.sendMsg(Team.type.OtherSmurf | Team.type.BoSmurf, "stop");
				Team.clickWP();
		...
		this.tombs = function () {
			...
			for (i = 66; i <= 72; i += 1) {
				if (Team.myType === Team.type.TeleSmurf) {
					Util.OtherSmurfToExit(i, false, Config.ClearType);
					if (tpTome.getStat(70)) {
						Pather.makePortal();
					}
					delay(2000);
				}
				Util.OtherSmurfToExit(i, true, Config.ClearType);
				unit = false;
		...
		this.duriel = function () {
			...
			if (!me.getQuest(14, 1) && !me.getQuest(14, 3)
				...
				if (Team.myType === Team.type.TeleSmurf) {
					...
					Util.OtherSmurfToExit(getRoom().correcttomb, true);
					for (i = 0; i < 3; i += 1) {
						try {
		...
		if (!me.getQuest(10, 0) && Team.myType === Team.type.TeleSmurf) {
			// Util.travel(4);
	...
	act3: function () {
		...
		this.heart = function () {
			...
			Util.OtherSmurfToExit([92, 93], true);
		...
		this.brain = function () {
			...
			Util.OtherSmurfToExit([88, 89, 91], true);
			try {
		...
		this.mephisto = function () {
			...
			if (Team.myType === Team.type.TeleSmurf) {
				Pather.useWaypoint(101, true);
				Precast.doPrecast(true);
				Util.OtherSmurfToExit(102, true);
		...
		} else if (Team.myType === Team.type.BoSmurf
	...
	act4: function () {
		this.izual = function () {
			...
			if (!me.getQuest(25, 1)) {
				if (Team.myType === Team.type.TeleSmurf) {
					Precast.doPrecast(true);
					Util.OtherSmurfToExit([104, 105], true);
	...
	act5: function () {
		...
		this.rescueBarbs = function () {
			...
			if (!me.getQuest(36, 1) && Team.myType === Team.type.TeleSmurf) {
				Pather.teleport = true;
				Pather.useWaypoint(111, true);
				Precast.doPrecast(true);
		...
		this.anya = function () {
			...
			if (!me.getQuest(37, 1) && Team.myType === Team.type.TeleSmurf) {
				Town.move("malah");
				...
				Pather.teleport = true;
				Config.ClearType = false;
				Util.OtherSmurfToExit(114, true, Config.ClearType);
				if (me.area !== 114) {
					Util.OtherSmurfToExit(114, true, Config.ClearType);
				}
				Team.inGame();
		...
		this.ancients = function () {
			...
			if (Team.myType === Team.type.TeleSmurf) {
				...
				Util.OtherSmurfToExit(120, false);
				Pather.makePortal();
				Team.sendMsg(Team.type.OtherSmurf | Team.type.BoSmurf, "come");
			...
			Util.OtherSmurfToExit(120, true);
			...
			if (altar && Team.myType === Team.type.BoSmurf) {
			...
			if (Team.myType === Team.type.TeleSmurf
				...
				Util.OtherSmurfToExit([128, 129], true);
				Team.clickWP();
				Town.goToTown();
			} else {
				if (!Util.OtherSmurfToExit(118, true)) {
					for (i = 0; i < 120
							&& !me.getQuest(39, 0) && !me.getQuest(39, 1);
							i += 1) {
						Attack.clear(40);
		...
		this.baal = function () {  //(<3 YGM)
			...
			if (Team.myType === Team.type.TeleSmurf) {
				...
				if (Team.isLevel(SmurfConfig.walkBaalLvl[me.diff])) {
					Pather.teleport = true;
					Util.OtherSmurfToExit([130, 131], true);
			...
			while (me.area !== 131) { //walking
				...
				if (me.area === 129 || me.area === 130) {
					if (!Util.OtherSmurfToExit(me.area + 1, true,
											Config.ClearType)) {
						Town.goToTown();
					}
				} else {
...
BaalLoop:
			while (true) {
				...
				} else if (Team.myType === Team.type.TeleSmurf) {
			...
			sendPacket(1, 0x40);
			delay(me.ping * 2);
			if (!Team.isLevel(SmurfConfig.killBaal[me.diff])) {
				return false;
			}
		...
		Town.goToTown(5);
		if (Team.myType === Team.type.TeleSmurf && !me.getQuest(39, 0)) {
			Util.travel(8);
		}
		...
		if (Team.myType === Team.type.TeleSmurf && !me.getQuest(40, 0)) {
			Util.travel(9);
		}
		return this.baal();
	},
...
function AutoSmurf() {
	addEventListener("copydata", function (mode, msg) {
		...
		case 100: //action
			...
			case "ping":
				if (Msg.y === 1) { //ready
					Team.sendMsg(Team.type.BoSmurf, "inc"); //pong
				}
				break;
			case "update":
				if (!me.inTown && getTickCount() - Msg.timer > 1000) {
					Msg.timer = getTickCount();
					Team.sendMsg(Team.type.OtherSmurf, "",
			...
			case "tp":
				...
				break;
			case "mephTP":
				if (Team.myType === Team.type.TeleSmurf && !Msg.area) {
					Msg.area = 1;
					delay(3000);
					Team.act3to4();
					Msg.area = 0;
		...
		case 105: //init Team.names
			if (msg[0] === "*") {
				Team.names.unshift(msg.substring(1)); //BoSmurf 0
			} else if (msg[0] === "$") {
				Team.names.splice(1, 0, msg.substring(1)); //TeleSmurf 1
			} else {
				Team.names.push(msg);
			}
			break;
	...
	addEventListener("chatmsg", function (nick, msg) {
		...
		if (/(\W+|^)(tp|portal|townportal|bo)(\W+|$)/i.test(msg)
			...
			if (getTickCount() - Msg.timer > 5000) {
				...
				&& Team.myType === Team.type.BoSmurf && !me.inTown) {
			if (getTickCount() - Msg.timer > 5000) {
				Msg.timer = getTickCount();
				say("come...");
				say("!here! :D");
			...
			if (Team.myType === Team.type.BoSmurf && nick !== me.name &&
				...
				&& Team.myType === Team.type.BoSmurf) {
			say("Yeah well sorry, I'm just a OtherSmurf... I mean, an AutoSmurf!");
		}
	});

	if (Team.init()) {
		while (Quest.launch()) {
			me.overhead("OtherSmurf");
			delay(500 + me.ping * 2);
		}
	}
}